...
impl FileFinder {
    ...
    fn refresh_picker_after_worktree_update(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        self.refresh_after_worktree_update = Some(cx.spawn_in(window, async move |file_finder, cx| {
            cx.background_executor()
                .timer(WORKTREE_UPDATE_REFRESH_DEBOUNCE)
                .await;
            file_finder
                .update_in(cx, |file_finder, window, cx| {
                    file_finder.refresh_after_worktree_update = None;
